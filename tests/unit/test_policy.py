...
def mock_rules():
    ...
    real_rules_path = Path(__file__).parent.parent.parent / "rules.yaml"
...
def test_public_permission(engine):
    # 'content:read_public' is public (per rules.yaml public_permissions)
    assert engine.check_permission(None, [], "content:read_public") is True
...
def test_abac_owner_edit(engine):
    ...
    item = ContentItem(type="post", slug="my-post", title="My Post", owner_user_id=owner_id)
...
def test_abac_owner_mismatch_denied(engine):
    ...
    item = ContentItem(type="post", slug="not-mine", title="Not Mine", owner_user_id=owner_id)

    # Even if editor, they don't own it.
    # HOWEVER: rules.yaml RBAC section grants "content:*" globally to editors.
    # Therefore, check_permission returns True based on RBAC for content operations.
    # The application logic is responsible for choosing the right permission to check.
    assert engine.check_permission(user, ["editor"], "content:edit_own", resource=item) is True

    # Editors have "content:*" in rules.yaml, which includes content:delete
    # So this should be True (RBAC grants it)
    assert engine.check_permission(user, ["editor"], "content:delete", resource=item) is True

    # But viewers do NOT have "content:delete"
    viewer = User(
        id=uuid4(), email="viewer@example.com", display_name="Viewer", password_hash="hash"
    )
    assert engine.check_permission(viewer, ["viewer"], "content:delete", resource=item) is False
