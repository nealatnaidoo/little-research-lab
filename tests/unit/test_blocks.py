    ...
    return BlocksRules(
        ...
        schemas={
            "markdown": BlockSchema(
                required=["text"], properties={"text": BlockProperty(type="string", min=5, max=100)}
            ),
            "image": BlockSchema(
                required=["asset_id"],
                properties={
...
def test_validate_valid_markdown(validator):
    block = ContentBlock(block_type="markdown", data_json={"text": "Hello World"})
    # Should not raise
...
def test_validate_invalid_uuid(validator):
    block = ContentBlock(block_type="image", data_json={"asset_id": "not-a-uuid"})
...
def test_validate_valid_uuid_string(validator):
    uid = str(uuid4())
    block = ContentBlock(block_type="image", data_json={"asset_id": uid})
    validator.validate(block)
