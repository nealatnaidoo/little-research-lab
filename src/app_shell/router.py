...
class Router:
    ...
    def register(self, route: str, builder: Callable[..., ft.View], protected: bool = True) -> None:
        self.routes[route] = RouteConfig(builder, protected)

    def register_dynamic(
        self, pattern: str, builder: Callable[..., ft.View], protected: bool = True
    ) -> None:
    ...
    def handle_route_change(self, e: ft.RouteChangeEvent) -> None:
        ...
        if not config:
            ...
            self.page.views.append(
                ft.View(
                    "/404", [ft.AppBar(title=ft.Text("404")), ft.Text(f"Page not found: {route}")]
                )
            )
            self.page.update()
            return
