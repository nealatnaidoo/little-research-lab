    ...
    def on_upload_result(e: ft.FilePickerResultEvent) -> None:
        ...
        for f in e.files:
            try:
                ...
                page.snack_bar = ft.SnackBar(ft.Text(f"Error uploading {f.name}: {err}"))
                page.snack_bar.open = True
        ...
        if asset.mime_type.startswith("image/"):
            ...
            if b64_data:
                preview_control = ft.Image(
                    src_base64=b64_data, width=50, height=50, fit=ft.ImageFit.CONTAIN
                )
        ...
        rows.append(
            ft.DataRow(
                cells=[
                    ...
                    ft.DataCell(
                        ft.IconButton(ft.Icons.DELETE, data=str(asset.id), on_click=on_delete)
                    ),
                ]
            )
        )
