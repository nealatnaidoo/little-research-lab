...
def RedeemInviteView(page: ft.Page, ctx: ServiceContext, state: AppState, token: str) -> ft.View:
    ...
    confirm_field = ft.TextField(label="Confirm Password", width=300, password=True)
    ...
    def on_submit(e: ft.ControlEvent) -> None:
        ...
        try:
            ...
            user = ctx.invite_service.redeem_invite(
                token, email_field.value, name_field.value, password_field.value
            )
            ...
            ft.Container(
                content=ft.Column(
                    [
                        ...
                        ft.Text(
                            "Create your account to join the lab.", size=14, color=ft.colors.OUTLINE
                        ),
                        email_field,
                        name_field,
                        password_field,
